/**
 * retrieve_question_bank Tool — 检索参考面试题
 *
 * 从本地 JSON 题库（data/knowledge/question-bank.json）做关键词匹配。
 * 不调 LLM，纯本地检索，给 generate_question 提供参考。
 */
...
export const retrieveQuestionBankTool = tool(
  async ({ query, job_role }) => {
    ...
    // retrieveQuestionBank 在 rag/retriever.ts，按标签打分取 top3
